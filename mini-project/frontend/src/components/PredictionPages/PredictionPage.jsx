...
async function enhanceWithGemini(symptoms, backendResult, retries = 2) {
  const prompt = `You are an intelligent medical assistant integrated into a disease prediction system.
Your role is to analyze symptoms and provide the most accurate disease prediction.

IMPORTANT RULES:
- Do NOT mention Gemini, Google API, AI model, or any external service.
- Do NOT say "based on AI", "generated by AI", or similar.
- The response must feel like it is generated by the system itself.
- Be confident and direct.

INPUT:
Symptoms: ${symptoms.join(", ")}
Initial Prediction: ${backendResult.disease}

TASK:
1. Carefully analyze the symptoms.
2. Validate whether the initial prediction is correct.
3. If correct → keep it.
4. If incorrect → replace it with a more accurate disease.
5. Prefer common and realistic diseases (avoid rare/complex ones).
6. If symptoms are unclear or serious → return: "Consult a doctor".

OUTPUT FORMAT (STRICT JSON ONLY):
{"disease":"Final Disease Name","confidence":"High / Medium / Low","reason":"Short and clear explanation (1-2 lines)","precautions":["point1","point2","point3"],"medications":["med1","med2","med3"],"diet":["diet1","diet2"],"workout":["tip1","tip2"]}

LOGIC GUIDELINES:
- Fever + body pain → Viral Fever
- Sneezing + runny nose + itchy eyes → Allergy
- Fever + chills + body ache → Flu
- Skin symptoms (pimples, oily skin) → Acne
- Rash + fever → Viral/Dengue (if mild → viral, else consult doctor)
- If mixed or risky symptoms → "Consult a doctor"
    ...
    try {
      ...
      if (backendData.disease) {
        ...
        try {
          ...
          // Gemini returns: disease, reason, precautions, medications, diet, workout
          setResult({
            ...backendData,
            disease: geminiResult.disease || backendData.disease,
            description: geminiResult.reason || backendData.description,
            ...
            diets: (geminiResult.diet || geminiResult.diets)?.length
              ? (geminiResult.diet || geminiResult.diets)
              : backendData.diets,
            workouts: (geminiResult.workout || geminiResult.workouts)?.length
              ? (geminiResult.workout || geminiResult.workouts)
              : backendData.workouts,
          });
          setGeminiCorrected(true);
        } catch (geminiErr) {
          console.error("Gemini enhancement failed:", geminiErr);
          // Show ML result but flag that Gemini didn't enhance it
          setResult({ ...backendData, geminiError: true });
        } finally {
      ...
      <div className="container">
        ...
        {(loading || geminiValidating) && (
          <div className="loading-container">
            ...
            <p>
              {geminiValidating
                ? "✦ Validating and enhancing your results..."
              ...
              <div className="disease-info">
                ...
                {result.geminiError && (
                  <span style={{
                    ...
                    ⚠ Gemini unavailable — showing ML result
                  </span>
                )}
