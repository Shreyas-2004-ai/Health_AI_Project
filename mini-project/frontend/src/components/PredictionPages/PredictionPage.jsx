...
async function enhanceWithAI(apiBase, symptoms, backendResult) {
  const response = await axios.post(`${apiBase}/api/enhance`, {
    symptoms,
...
const PredictionPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formattedSymptoms = symptomList.map((symptom) => {
        ...
        const directMatch = allSymptoms.find(
          (s) =>
        ...
        for (const [originalSymptom, synonyms] of Object.entries(
          symptomSynonyms
        )) {
          if (
            synonyms.some((syn) => syn.toLowerCase() === symptom.toLowerCase())
          ) {
      ...
      // AI enhancement — runs before showing any result
      if (backendData.disease) {
        try {
          ...
          setResult({
            ...backendData,
            disease: aiResult.disease || backendData.disease,
            description: aiResult.reason || backendData.description,
            precautions: aiResult.precautions?.length ? aiResult.precautions : backendData.precautions,
            medications: aiResult.medications?.length ? aiResult.medications : backendData.medications,
            diets: (aiResult.diet || aiResult.diets)?.length ? (aiResult.diet || aiResult.diets) : backendData.diets,
            workouts: (aiResult.workout || aiResult.workouts)?.length ? (aiResult.workout || aiResult.workouts) : backendData.workouts,
          });
        } catch (err) {
  ...
  const sendFeedback = async () => {
    if (!result || !result.disease || lastFormattedSymptoms.length === 0)
      return;
    try {
            ...
            <div className="result-card">
              ...
              <div className="result-footer">
                <p className="confidence-note">
                  ...
                  {result.prediction_method === "improved_ml"
                    ? "High"
                    : "Medium"}
                </p>
                <p className="medical-disclaimer">
                  * This is an AI-powered prediction based on the symptoms you
                  provided. Please consult with a healthcare professional for
